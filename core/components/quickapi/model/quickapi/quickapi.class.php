...
class QuickApi
{
    ...
    public function checkPermissions()
    {
        ...
        /**
         * Check for authorization overrides
         * 
         * Checks first for a specific authorization snippet following the
         * format, Api<MethodName>Authorized. It then falls back on the
         * format, MyApiAuthorized, and finally the standard ApiAuthorized.
         * 
         */
        // Set the default name (final fallback)
        $snippetName = 'ApiAuthorized';
        $formatOne = 'Api'.$this->service.'Authorized';
        $formatTwo = 'MyApiAuthorized';
        
        // Check for a specific 
        if ($this->modx->getObject('modSnippet', ['name' => $formatOne])) {
            $snippetName = $formatOne;
        }
        else if ($this->modx->getObject('modSnippet', ['name' => $formatTwo])) {
            $snippetName = $formatTwo;
        }
        
        // Attempt to run an auth snippet
        $snippetAuth = $this->modx->runSnippet($snippetName, [
            "quickapi" => $this,
    ...
    public function process() 
    {
        ...
        if ($snippet = $this->modx->getObject('modSnippet', ['name' => 'Api'.$this->service])) {
            ...
            // No snippet was found by the matching name, return a 404 not found message
